<?php
return [
    'menu_header' => [
        'home'              => 'Trang chủ',
        'our_services'      => 'Dịch vụ',
        'our_projects'      => 'Dự án',
        'photography'       => 'Nhiếp ảnh',
        'design'            => 'Thiết kế',
        'video_editing'     => 'Chỉnh sửa video',
        'gallery'           => 'Thư Viện',
        'contact_us'        => 'Liên hệ',
    ],
    'hero_banner' => [
        'title'         => 'Biến nội thất của bạn thành tác phẩm nghệ thuật',
        'subtitle'      => 'Dịch vụ chụp & chỉnh sửa ảnh chuyên nghiệp cho không gian sống và làm việc',
        'cta'           => 'Xem Thư Viện',
    ],
     'about_company' => [
        'about_company'     => '[ Về Chúng Tôi ]',
        'title'             => 'Dịch Vụ Nhiếp Ảnh Toàn Diện',
        'description'       => 'Chúng tôi là một tập thể chuyên nghiệp, quy tụ những thành viên giàu kinh nghiệm trong lĩnh vực chụp và hậu kỳ hình ảnh sản phẩm nội thất. 
                                Với nhiều năm đồng hành cùng các thương hiệu gỗ uy tín hàng đầu tại Việt Nam, chúng tôi tự hào mang đến những 
                                hình ảnh sản phẩm nội thất sắc nét, tinh tế và đạt chuẩn thẩm mỹ cao.',
    ],
    'our_services' => [
        'big_title'         => 'DỊCH VỤ CỦA CHÚNG TÔI',
        'title_1'           => 'CHỤP ẢNH SẢN PHẨM NỘI THẤT',
        'description_1'     => 'Sử dụng thiết bị tốt nhất để mang lại kết quả hoàn hảo.',
        'title_2'           => 'CHỈNH SỬA ẢNH CHUYÊN NGHIỆP',
        'description_2'     => 'Đáp ứng mọi yêu cầu - Cam kết chất lượng & tiến độ.',
        'title_3'           => 'KẾT HỢP AI & ẢNH CHỤP',
        'description_3'     => 'Đón đầu xu hướng hình ảnh mới, chúng tôi mang đến những bức ảnh lifestyle độc đáo nhờ sự kết hợp giữa công nghệ AI và sáng tạo thực tế.',
    ],
    'home_gallery' => [
        'title'         => 'Thư Viện',
        'description'   => 'Bộ sưu tập những tác phẩm xuất sắc',
        'tab_1'         => 'Chụp Ảnh Nội Thất',
        'tab_2'         => 'Kết Hợp AI',
        'tab_3'         => 'Chỉnh Ảnh Chuyên Nghiệp',
    ],  
    'contact' => [
        'banner-title'       => 'Liên hệ với chúng tôi',
        'banner-subtitle'    => 'Sẵn sàng biến ý tưởng của bạn thành hiện thực? Hãy cùng thảo luận về nhu cầu chụp ảnh và nội thất của bạn.',
        'banner-cta'         => 'Liên hệ ngay hôm nay',
        'big_title'          => 'Liên Hệ',
        'address'            => 'Địa Chỉ',
        'phone'              => 'Điện Thoại',
        'email'              => 'Email',
        'social'             => 'Mạng Xã Hội',
        'page'               => 'Trang'
    ],
    'gallery' => [
        'banner-title'       => 'Thư Viện Của Chúng Tôi',
        'banner-subtitle'    => 'Bộ sưu tập những tác phẩm xuất sắc',
        'banner-cta'         => '',
    ],
    
];
